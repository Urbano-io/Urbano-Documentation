...
def exportDescription(component, pluginName, githubFolder, githubRepo=None):
    ...
    if githubRepo:
        source = f" - [[source code :material-open-in-new:]({githubRepo}/{originalName.replace(' ', '%20')}.cs){{ target=\"_blank\" rel=\"noopener noreferrer\" aria-label=\"source code (opens in a new tab)\" title=\"View C# source code on GitHub\" }}]\n"
